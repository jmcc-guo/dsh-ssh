...
const en = {
  ...
  tunnel: 'SSH tunnel',
  tunnelNone: 'None — direct connection',
  tunnelHint: 'Connect through another saved connection (jump host).',
  connect: 'Connect',
...
const zh = {
  ...
  tunnel: 'SSH 隧道（跳板）',
  tunnelNone: '无 —— 直连',
  tunnelHint: '通过另一条已保存的连接作为跳板（SSH 隧道）连接目标主机。',
  connect: '连接',
  disconnect: '断开',
  reconnect: '重连',
  delete: '删除',
...
function TerminalView({ t, tab, lines, current, name }) {
  ...
  const aiBusy = busyBy === 'ai';

  // Sticky-to-bottom: `pinned` mirrors whether the user is at (or within a
  // small margin of) the bottom of the scrollback. Scrolling up releases the
  // pin so history stays readable; returning to the bottom re-arms it. While
  // the AI drives this connection, every AI operation re-pins the view at
  // the newest output, so the terminal slides to the bottom whenever the AI
  // acts on the connection.
  const pinnedRef = useRef(true);
  const boundTabRef = useRef(null);

  // Track the user's own scroll intent on the scrollback element.
  useEffect(() => {
  ...
  // The scroll element persists across tab switches (the view is not keyed):
  // a newly bound tab starts pinned to its own bottom, and an AI operation
  // starting on the bound tab re-pins it at the latest output.
  useEffect(() => {
    if (boundTabRef.current !== name) {
      boundTabRef.current = name;
  ...
  // Follow the newest output while pinned. The pin is decided by the user's
  // scroll position (or the AI re-pin) BEFORE the new content grows the
  // scroll area, so a large output burst cannot detach the view from the
  // bottom the way a post-growth distance check would.
  useEffect(() => {
    const el = scrollRef.current;
    if (el && pinnedRef.current) el.scrollTop = el.scrollHeight;
  }, [lines.length, current, aiBusy, name]);
...
function ConnectionForm({ t, initial, submitLabel, onSubmit, onCancel, busy, error, showSaveAndConnect }) {
  ...
  return h('div', { className: 'ssh-form' }, [
    ...
    h('label', { className: 'ssh-field' }, [
      h('span', { className: 'ssh-field-label' }, t('tunnel')),
      h('select', { className: 'ssh-input', value: tunnel, onChange: (event) => setTunnel(event.target.value) }, [
        h('option', { value: '' }, t('tunnelNone')),
        ...records
          .filter((record) => record.name !== (initial?.name ?? ''))
          .map((record) => h('option', { key: record.name, value: record.name }, `${record.name} (${record.user}@${record.host}:${record.port})`)),
      ]),
      h('span', { className: 'ssh-field-hint' }, t('tunnelHint')),
    ]),
    authType === 'password'
...
function SettingsRow({ t, record }) {
  ...
  return h('div', { className: 'ssh-settings-row' }, [
    ...
    h('div', { className: 'ssh-settings-row-meta' },
      `${record.user}@${record.host}:${record.port} · ${record.authType === 'password' ? t('authPassword') : t('authKey')}${record.tunnel ? ' · via ' + record.tunnel : ''}${record.busyBy ? ' · ' + (record.busyBy === 'ai' ? t('busyAi') : t('busyUser')) : ''}`),
...
.ssh-field-hint{font-size:10px;color:var(--dsw-alias-fg-muted,#77736d);text-transform:none;letter-spacing:0;font-weight:400}
