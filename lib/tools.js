/**
 * @jmcc-guo/dsh-ssh — model-facing tools.
 *
  ...
  return [
    defineTool({
      ...
      description: 'Establish an SSH connection managed by the SSH terminal panel. '
        ...
        + 'A new connection may route through an existing saved connection (`tunnel`) as an SSH tunnel / jump host. '
      ...
      parameters: {
        ...
        tunnel: { type: 'string', description: 'Name of an existing saved connection to use as an SSH tunnel (jump host) for the new connection; omit for a direct connection.' },
        auth: AUTH_SCHEMA,
