window.__ModuleLoader__.load({ id: "@dsh-external/dsh-ssh", factory: (require) => {
var module = { exports: {} }; var exports = module.exports;
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });

/**
 * @dsh-external/dsh-ssh — browser half.
 *
 * Three surfaces:
...
function FormModal({ t }) {
  ...
  const onSubmit = (fields) => {
    ...
    const params = { ...fields };
    if (editing) params.name = editing;
  ...
  return h('div', { className: 'ssh-modal-mask', onClick: busy ? undefined : close }, [
    h('div', { className: 'ssh-modal', onClick: (event) => event.stopPropagation() }, [
      ...
      h(ConnectionForm, {
        t, initial: record, submitLabel: editing ? t('save') : t('createAndConnect'),
...
function ConnectionForm({ t, initial, submitLabel, onSubmit, onCancel, busy, error }) {
  ...
  return h('div', { className: 'ssh-form' }, [
    field(t('name'), name, setName, 'serverA', { disabled: initial !== undefined }),
    ...
    h('div', { className: 'ssh-form-actions' }, [
      ...
      h('button', { type: 'button', className: 'ssh-btn ssh-btn-primary', disabled: busy, onClick: submit }, busy ? '\u2026' : submitLabel),
    ]),
  ]);
}
...
function SettingsRow({ t, record }) {
  ...
  return h('div', { className: 'ssh-settings-row' }, [
    ...
    h('div', { className: 'ssh-settings-row-actions' }, [
      ...
      record.source === 'user'
        ? h('button', { type: 'button', className: 'ssh-btn ssh-btn-mini ssh-btn-transfer', onClick: () => store.set({ transferName: record.name, transferDone: false }) }, t('transferToAi'))
        : null,
...
function installStyles() {
  const id = '@dsh-external/dsh-ssh/client';
  const existing = document.querySelector(`style[data-plugin-css="${id}"]`);
  if (existing !== null) return () => {};
  const style = document.createElement('style');
  style.dataset.plugin = '@dsh-external/dsh-ssh';
